...
remove_stale_app_images() {
    ...
    while IFS= read -r image_id; do
        ...
        for ref in $refs; do
            case "$ref" in
                ghcr.io/ktb-loadtest-15/ktb-bootcampchat-frontend:*)
...
remove_all_but_one_existing_app_image() {
    ...
    if [ -n "$keep_ref" ]; then
        log_info "Keeping one existing frontend image before pull: $keep_ref"
    else
        log_info "No active frontend image to preserve before pull"
    fi
