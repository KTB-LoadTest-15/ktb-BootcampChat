...
class MessageReadStatusServiceTest {
    ...
    private MessageReadStatusService service() {
        return new MessageReadStatusService(messageStore);
    }

    @Test
    @DisplayName("읽음 처리는 메시지 개수와 무관하게 store bulk update 1회로 위임한다")
    void updateReadStatus_delegatesToSingleBulkUpdate() {
        List<String> ids = List.of("m1", "m2", "m3");
        when(messageStore.addReaderToMessages(anyList(), anyString(), any(LocalDateTime.class)))
                .thenReturn(3L);

        service().updateReadStatus(ids, "user-1");

        // 개별 findById/update가 아니라 단일 호출이어야 한다 (N+1 제거의 핵심)
        verify(messageStore, times(1))
                .addReaderToMessages(eq(ids), eq("user-1"), any(LocalDateTime.class));
        verify(messageStore, never()).findById(anyString());
    ...
    void updateReadStatus_emptyList_noDbCall() {
        ...
        verify(messageStore, never())
                .addReaderToMessages(anyList(), anyString(), any(LocalDateTime.class));
    }
    ...
    @Test
    @DisplayName("store 예외는 삼켜서 호출자 흐름을 깨지 않는다")
    void updateReadStatus_swallowsStoreException() {
        when(messageStore.addReaderToMessages(anyList(), anyString(), any(LocalDateTime.class)))
