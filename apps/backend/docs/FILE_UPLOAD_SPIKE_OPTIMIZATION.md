...
S3 운영 모드에서 프로필 이미지도 `Presign 발급 -> S3 PUT -> 프로필 확정`
순서로 전환했다. 기존 `POST /api/users/profile-image` multipart 계약은 로컬
저장소와 기존 클라이언트를 위해 유지한다. 운영 프론트의 최종 확정 응답도
같은 URL에서 `imageUrl`을 반환하므로 기존 E2E 응답 계약을 유지한다.
