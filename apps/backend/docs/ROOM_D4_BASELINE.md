...
- 애플리케이션에서 전체 응답 DTO를 만든 뒤 정렬하던 작업을 MongoDB의 `createdAt DESC` 정렬 조회로 이동했다.
- 이번 변경에서는 인덱스를 추가하지 않고 정렬 책임만 애플리케이션에서 MongoDB로 이동한다.
...
## D4-5 단일 방 응답 사용자 N+1 개선

- 방 상세·생성·참여 응답에서 생성자와 참가자를 각각 조회하던 코드를 사용자 ID 일괄 조회로 변경한다.
- 생성자 포함 참가자 11명인 방 기준 repository 조회는 약 14회에서 3회로 감소한다.
- 방 조회 1회, 사용자 일괄 조회 1회, 최근 메시지 count 1회로 구성된다.
- API URL과 응답 구조는 유지한다.

생성자 포함 참가자 11명인 동일 방에서 워밍업 3회 후 `GET /api/rooms/{roomId}`를 순차 30회 호출했다.

| 버전 | repository 조회 예측 | 성공 | 평균 | p90 | 최대 |
|---|---:|---:|---:|---:|---:|
| 기존 개별 사용자 조회 | 약 14회 | 30/30 | 8.49ms | 9.78ms | 11.76ms |
| 사용자 일괄 조회 | 약 3회 | 30/30 | 6.19ms | 7.03ms | 11.00ms |

- 평균 약 27.1%, p90 약 28.1%, 최대 약 6.5% 감소했다.
- 로컬 순차 호출의 간단 비교이며 한계 처리량 결과는 아니다.

## D4 현재 코드 최종 간이 측정

동일 로컬 서버와 DB에서 워밍업 후 각 API를 순차 30회 호출했다.

| API | 성공 | 평균 | p90 | 최대 |
|---|---:|---:|---:|---:|
| `GET /api/rooms` | 30/30 | 11.85ms | 15.13ms | 18.38ms |
| `GET /api/rooms/{roomId}` | 30/30 | 5.74ms | 6.82ms | 8.41ms |

- 서로 다른 시점의 로컬 간이 측정이므로 이 값만으로 한계 TPS 개선을 확정하지 않는다.
- 방 목록 사용자 조회와 단일 방 사용자 조회의 N+1 제거가 현재 코드에 반영된 상태를 확인하기 위한 참고값이다.

### 변경 후 E2E 회귀 확인

E2E 파일을 수정하지 않고 README의 localhost 조건으로 전체 스위트를 실행했다.

```bash
cd e2e
BASE_URL=http://localhost:3000 pnpm run test:full
```

- 결과: 20 passed, 2 failed, 13 did not run
- 두 실패 모두 `loginAction()`의 `page.goto('/login')`에서 `net::ERR_ABORTED`가 발생했다.
- 실패 위치는 `auth.spec.js` 회원가입 후 로그인과 `chat.spec.js` 사전 로그인이다.
- 기존 기준선에서 재현된 로그인 페이지 이동 오류와 같은 유형이며, 방 API 응답 검증 실패는 확인되지 않았다.
- 전체 E2E 안정성 게이트는 여전히 미통과이므로 D4 완료 판정과 별개로 기존 E2E 불안정 원인을 해결해야 한다.
